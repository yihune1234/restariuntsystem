...
import {
  DollarSign, ShoppingCart, LayoutGrid, Wallet, Clock, AlertTriangle,
  TrendingUp, UtensilsCrossed, Activity, ArrowRight, RefreshCw, Loader2,
...
const OwnerDashboard = () => {
  ...
  const {
    ownerKPIs, hourlySales, foodReport,
    isLoading, isFetchingCharts, fetchAllDashboardData, listenForRealTimeUpdates,
  } = useDashboardStore();
  ...
  const paymentPieData = useMemo(() => {
    const byMethod = paymentBreakdown ? Object.entries(paymentBreakdown).map(([key, val]) => ({
      name: key,
      value: val.amount || 0,
    })).filter(d => d.value > 0) : [];
    return byMethod;
    ...
    <div className="p-4 lg:p-6 space-y-6 max-w-7xl mx-auto">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold tracking-tight flex items-center gap-2">
            <Activity className="size-6 text-primary" /> Business Overview
          </h1>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
        ...
        <SummaryCard
          ...
          value={`${(revenueKpis.unpaid || 0).toLocaleString()} ETB`}
        ...
        <SummaryCard
          ...
          value={orders.pending || orders.unpaid || 0}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-lg flex items-center gap-2">
              <Wallet className="size-5 text-primary" /> Payments by Method
            </CardTitle>
            <CardDescription>Cash, Card, Digital</CardDescription>
          </CardHeader>
          <CardContent className="h-64">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <Card>
          <CardHeader className="pb-2">
            ...
            <CardDescription>Create orders & manage the restaurant</CardDescription>
          </CardHeader>
          <CardContent>
            <div className="grid grid-cols-2 gap-2">
              <QuickActionButton icon={ShoppingCart} label="New Order" onClick={() => navigate("/owner/create")} />
              <QuickActionButton icon={Clock} label="All Orders" onClick={() => navigate("/owner/orders")} />
              <QuickActionButton icon={UtensilsCrossed} label="Menu" onClick={() => navigate("/owner/menu")} />
              <QuickActionButton icon={Users} label="Employees" onClick={() => navigate("/owner/employees")} />
              <QuickActionButton icon={LayoutGrid} label="Tables" onClick={() => navigate("/owner/tables")} />
              <QuickActionButton icon={BarChart3} label="Reports" onClick={() => navigate("/owner/reports")} />
            </div>
          </CardContent>
        </Card>
      </div>
