...
class MealPeriodController {
  ...
  getMealPeriods = asyncHandler(async (req, res) => {
    ...
    const mealPeriods = await mealPeriodService.getMealPeriods({
  ...
  reorderMealPeriods = asyncHandler(async (req, res) => {
    const { orders } = req.body;
    if (!Array.isArray(orders)) {
      return ApiResponse.error(res, 400, 'orders must be an array of { id, displayOrder }');
    }
    const result = await mealPeriodService.reorderMealPeriods(orders);
    return ApiResponse.success(res, 200, result.message);
  });
}
